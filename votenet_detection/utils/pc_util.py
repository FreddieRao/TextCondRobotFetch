...
def get_pc_in_bounds(pc, scene_bbox, pred_confident_nms_class, out_filepath):
    """Export point cloud in the bbx
    Args:
        pc: point cloud of the scene
        bound" bound of bounding box
        out_filename: (string) filename
    """
    def heading2rotmat(heading_angle):
        pass
    ...
    def get_obb_bound(box):
        ...
        trns[0:3,0:3] = heading2rotmat(box[6])
        box_trimesh_fmt = trimesh.creation.box(lengths, trns)
        return trns, box_trimesh_fmt
    
    bound_list = []
    trns_list = []
    for box in scene_bbox:
        trns, bbx = get_obb_bound(box)
        bound_list.append(bbx)     
        trns_list.append(trns)    

    pc_coord = pc[:, 0:3]

    for b_idx, (trns, bbx) in enumerate(zip(trns_list, bound_list)):
        bb_filter = bbx.contains(pc_coord)
        pcs = pc_coord[bb_filter] #[n, 3]
        pcs = np.concatenate([pcs, np.ones((pcs.shape[0], 1))], axis=-1) #[n, 4]
        pcs_trans = pcs @ trns
        pcs_trans = pcs_trans[:, 0:3]
