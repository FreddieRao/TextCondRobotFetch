...
@torch.no_grad()
def generate_conditioned(best_model, labels=None, device='cuda', n_row=1, out_dim=256):
    best_model.eval()
    if labels==None:
        samples = best_model.sample(num_samples=n_row, noise=None)
    else:
        all_samples_list = []
        for _ in range(n_row):
            samples = best_model.sample(num_samples=1, noise=None, cond_inputs=labels, out_dim=out_dim)
            all_samples_list.append(samples)
        samples = torch.cat(all_samples_list, dim=0)
    return samples
...
class FlowSequential(nn.Sequential):
    ...
    def sample(self, num_samples=None, noise=None, cond_inputs=None, out_dim=256):
        if noise is None:
            noise = torch.Tensor(num_samples, out_dim).normal_()
...
class Feedforward(torch.nn.Module):
        def __init__(self, input_size, hidden_size):
            ...
            self.norm1 = torch.nn.BatchNorm1d(self.hidden_size)
            self.fc2 = torch.nn.Linear(self.hidden_size, self.hidden_size)
            self.relu2 = torch.nn.LeakyReLU()
            self.norm2 = torch.nn.BatchNorm1d(self.hidden_size)
