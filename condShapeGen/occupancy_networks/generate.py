...
cfg = config.load_config(args.config, 'occupancy_networks/configs/default.yaml')
...
generate_embed = cfg['generation']['generate_embed']
...
if generate_embed and not hasattr(generator, 'generate_embed'):
    generate_embed = False
    print('Warning: generator does not support generate embed.')
...
for it, data in enumerate(tqdm(test_loader)):
    ...
    if category_id != 'n/a':
        ...
        embed_dir = os.path.join(embed_dir, str(category_id))
    ...
    if generate_embed and not os.path.exists(embed_dir):
    ...
    if generate_embed:
        t0 = time.time()
        embed = generator.generate_embed(data).squeeze(0).cpu().numpy()
        latent = generator.generate_latent(data).squeeze(0).cpu().numpy()
        embed = np.concatenate([latent, embed], axis=0)
        embed_feats[modelname] = embed
        time_dict['embed'] = time.time() - t0
        # embed_out_file = os.path.join(
            # embed_dir, '%s.npz' % modelname)
        # out_file_dict['embed'] = embed_out_file
        # np.savez(embed_out_file, embed)
...
if generate_embed:
    embed_out_file = os.path.join(embed_dir, 'embed_feats_'+split+'.pickle')
    with open(embed_out_file, 'wb') as handle:
