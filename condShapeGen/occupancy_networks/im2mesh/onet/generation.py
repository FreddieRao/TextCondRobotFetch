...
class Generator3D(object):
    ...
    def generate_embed(self, data, return_stats=False):
        ''' Generates the output mesh.
    ...
    def generate_latent(self, data, return_stats=False):
        ''' Generates the output mesh.
        ...
        if self.model.encoder_latent is not None:
            with torch.no_grad():
                mean_z, logstd_z = self.model.encoder_latent(p, occ, c, **kwargs)


        if return_stats:
            return mean_z, stats_dict
        else:
            return mean_z
    ...
    def extract_mesh(self, occ_hat, z, c=None, stats_dict=dict()):
        ...
        if self.refinement_step > 0:
            t0 = time.time()
            mesh = self.refine_mesh(mesh, occ_hat, z, c)
