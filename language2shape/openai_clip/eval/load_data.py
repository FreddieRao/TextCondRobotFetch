...
def load_shape_embed_feats(args, split='val', load_valid_cates_only=False):
    ...
    with open(args.embed_feat_path.replace('embed_feats_train', 'embed_feats_test'), 'rb') as handle:
    ...
    with open(args.embed_feat_path.replace('embed_feats_train', 'embed_feats_val'), 'rb') as handle:
